            ...
            <div className="flex justify-center mb-12">
                <div className="bg-white/5 p-1 rounded-full border border-white/10 inline-flex relative">
                    <button
                        ...
                        className={`relative z-10 px-6 py-2 rounded-full text-sm font-bold tracking-wider transition-colors duration-300 ${!isAnnual ? 'text-black' : 'text-gray-400 hover:text-white'}`}
                    >
                        MONTHLY
                    </button>
                    <button
                        onClick={() => setIsAnnual(true)}
                        className={`relative z-10 px-6 py-2 rounded-full text-sm font-bold tracking-wider transition-colors duration-300 flex items-center gap-2 ${isAnnual ? 'text-black' : 'text-gray-400 hover:text-white'}`}
                    >
                        ANNUAL
                        <span className={`text-[9px] px-2 py-0.5 rounded-full font-mono ${isAnnual ? 'bg-black text-[#b71928]' : 'bg-[#b71928] text-white'}`}>SAVE 20%</span>
                    </button>
                    
                    {/* Active Pill Background */}
                    <div 
                        className={`absolute top-1 bottom-1 w-[calc(50%-4px)] bg-white rounded-full transition-transform duration-300 ease-in-out ${isAnnual ? 'translate-x-[calc(100%+4px)]' : 'translate-x-0'}`}
                        style={{ width: isAnnual ? '140px' : '108px' }}
                    />
                </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
                ...
                <div className="glass-panel p-8 rounded-2xl border border-white/5 bg-[#050505] hover:border-white/20 transition-all flex flex-col">
                    ...
                    <div className="space-y-4 flex-grow">
                        <div className="text-xs font-mono tracking-widest text-gray-500 mb-4 uppercase">Includes</div>
                ...
                <div className="glass-panel p-8 rounded-2xl border border-[#b71928]/50 bg-gradient-to-b from-[#b71928]/10 to-[#050505] relative flex flex-col hover:-translate-y-2 transition-all duration-300">
                    <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 py-1 bg-[#b71928] text-black text-[10px] font-bold font-mono tracking-widest rounded-full">
                        MOST POPULAR
                    </div>
                    <div className="mb-8 mt-2">
                        <h2 className="text-2xl font-bold font-vox mb-2">Pro</h2>
                        <p className="text-gray-400 text-sm h-10">For boutique firms requiring autonomous redlining.</p>
                        <div className="mt-6 flex items-baseline gap-1">
                            <span className="text-xl text-gray-500">$</span>
                            <span className="text-5xl font-bold">{isAnnual ? "24" : "29"}</span>
                            <span className="text-xl text-gray-500">{isAnnual ? "" : ".99"}</span>
                            <span className="text-gray-500 ml-1">/mo</span>
                        </div>
                        {isAnnual && <div className="text-xs text-gray-500 mt-1">Billed $288 annually</div>}
                    </div>
                    ...
                    <div className="space-y-4 flex-grow">
                        <div className="text-xs font-mono tracking-widest text-[#b71928] mb-4 uppercase">Everything in Starter, plus</div>
                ...
                <div className="glass-panel p-8 rounded-2xl border border-white/5 bg-[#050505] hover:border-white/20 transition-all flex flex-col">
                    <div className="mb-8">
                        ...
                        {isAnnual && <div className="text-xs text-gray-500 mt-1">Billed $1,428 annually</div>}
                    </div>
                    <Link href="/login" className="w-full block text-center py-3 rounded-lg border border-white/20 hover:bg-white hover:text-black transition-colors mb-8 font-bold text-sm tracking-wider">
                        UPGRADE TO FIRM
                    </Link>
                    <div className="space-y-4 flex-grow">
                        <div className="text-xs font-mono tracking-widest text-gray-500 mb-4 uppercase">Everything in Pro, plus</div>
                ...
                <div className="glass-panel p-8 rounded-2xl border border-white/5 bg-[#050505] hover:border-white/20 transition-all flex flex-col">
                    <div className="mb-8">
                        ...
                        {isAnnual && <div className="text-xs text-gray-500 mt-1">Billed $2,868 annually</div>}
                    </div>
                    <Link href="/login" className="w-full block text-center py-3 rounded-lg border border-white/20 hover:bg-white hover:text-black transition-colors mb-8 font-bold text-sm tracking-wider">
                        CONTACT SALES
                    </Link>
                    <div className="space-y-4 flex-grow">
                        <div className="text-xs font-mono tracking-widest text-gray-500 mb-4 uppercase">Everything in Firm, plus</div>
